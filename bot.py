...
def _parse_int_list(s: str) -> set[int]:
    ...
    for part in s.replace(" ", "").split(","):
        ...
        try:
            ...
            logging.warning("Skip bad ADMIN_IDS item: %r", part)
    return out

# Обязательные
BOT_TOKEN = _getenv_required("BOT_TOKEN")
ADMIN_CHANNEL_ID = int(_getenv_required("ADMIN_CHANNEL_ID"))

# Админы: поддерживаем и ADMIN_IDS (список), и старый ADMIN_ID (одиночный)
_admin_ids_env = os.getenv("ADMIN_IDS") or os.getenv("ADMIN_ID", "")
ALLOWED_ADMIN_IDS = _parse_int_list(_admin_ids_env) if _admin_ids_env else set()

# Опциональные
SUPPORT_USERNAME = os.getenv("SUPPORT_USERNAME", "MeuzenFC")
REVIEWS_URL = os.getenv("REVIEWS_URL", "https://t.me/+hP-T1TuoybA4YjA6")

CURRENCY = os.getenv("CURRENCY", "₽")
PRICE_PER_ACCOUNT = Decimal(os.getenv("PRICE_PER_ACCOUNT", "150.00"))

CARD_NUMBER = os.getenv("CARD_NUMBER", "2200 7010 2345 6789")
CARD_HOLDER = os.getenv("CARD_HOLDER", "Костышин С.Э")
CARD_NOTE   = os.getenv("CARD_NOTE", "Скриншот отправляй в бота!")

PORT = int(os.getenv("PORT", "10000"))  # Render отдаёт свой PORT
...
def main_menu():
    ...
    kb.button(text="⭐ Отзывы", url=REVIEWS_URL)  # сразу на канал отзывов
    kb.button(text="💬 Поддержка", url=f"https://t.me/{SUPPORT_USERNAME}")  # сразу в ЛС
    kb.adjust(1)
    return kb.as_markup()
